...
correct_loess_date_time <- function(df, model_quo) {
...
#' @param model a single regression model (usually \link[stats]{lm}, \link[stats]{glm} or \link[stats]{loess}) or a list of multiple alternative regression models for the calibration. If a named list is provided, the name(s) will be used instead of the formulaes for the model identification column. If multiple models are provided, the entire data table rows will be duplicated to consider the different models in parallel. Note that \link[stats]{loess} models do not work with \code{\link[base]{date-time}} variables (requiring them to be converted to a number instead). As this is a fairly common operation for \code{loess} models in this context (for temporal drift calibrations), this function will automatically inject the \code{as.numeric} correction into \code{loess} models whenever it encounters a dependent variable that is a \code{\link[base]{date-time}} unless parameter \code{auto_correct_loess_date_time = FALSE}.
...
#' @param auto_correct_loess_date_time whether to automatically inject the \code{as.numeric} call into \code{loess} models to enable datetime columns as dependent variables. There is usually no reason to turn this feature off even when not using any date-time \code{loess} models as it will not have any effect in this case.
...
#' @inheritParams unnest_model_column
#' @export
iso_unnest_calibration_coefs <- function(dt, calibration = last_calibration(dt), select = everything(), keep_remaining_nested_data = FALSE, keep_other_list_data = TRUE) {
  calib_vars <- get_calibration_vars(calibration)
...
#' @inheritParams unnest_model_column
#' @export
iso_unnest_calibration_summary <- function(dt, calibration = last_calibration(dt), select = everything(), keep_remaining_nested_data = FALSE, keep_other_list_data = TRUE) {
  calib_vars <- get_calibration_vars(calibration)
  check_calibration_cols(!!enquo(dt), calib_vars$model_params)
...
#' @inheritParams unnest_model_column
#' @export
iso_get_calibration_range <- function(dt, calibration = last_calibration(dt), select = everything(), keep_remaining_nested_data = FALSE, keep_other_list_data = FALSE) {
  calib_vars <- get_calibration_vars(calibration)
