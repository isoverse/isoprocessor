...
linearity_summary <- 
  ...
  mutate(
    fit = map(data, ~lm(`d13C [permil]` ~ `amp44 [V]`, data = .x)),
...
peak_table_analytes %>% 
  ...
  iso_plot_data(
    ...
    x = `area44 [Vs]`, y = c(`amp44 [V]`, `d13C [permil]`),
    # choose aesthetics
...
peak_table_analytes %>% 
  ...
  iso_plot_data(
    # x and y
    x = `injection [ng]`, y = c(`mean_area_identified [Vs]`, `mean_amp_identified [V]`),
    # aesthetics
...
global_calibs <- 
  ...
  iso_generate_calibration(
    ...
    is_std_peak = is_std_peak & area44 > 5 & area44 < 100 & seed_oxidation == "yes"
  )
```
...
peak_table_calibrated %>% 
  ...
  iso_plot_data(
    # choose x and y (multiple y possible)
    x = `area44 [Vs]`, y = `true_d13C_pred [permil]`,
    # choose aesthetics
