...
iso_files %>%
  iso_get_peak_table(include_file_info = c(seq_nr, analysis)) %>% 
  ...
  # visualize
  iso_plot_data(
    x = seq_nr, y = delta_vs_avg, fill = ref_info,
    geom_bar(stat = "identity", position = "dodge")
  ) +
  labs(
    x = "Sequence #",
    y = "Deviation from average (permil) within each analysis", 
    fill = "Reference\npeak") 
```
...
```{r "drift_vs_time", fig.width = 6, fig.height=7}
peak_table_w_standards %>% 
  filter(type == "drift") %>% 
  iso_plot_data(
...
```{r "linearity_vs_area", fig.width = 6, fig.height=7}
peak_table_w_standards %>% 
...
```{r "linearity_coefs", fig.width = 7, fig.height=9}
...
```{r "measured_vs_true", fig.width = 6, fig.height=7}
peak_table_lin_corr %>% 
...
Correct for isotopic scale contraction (discrimination) and offset: 
...
```{r "scale_residuals", fig.width = 8, fig.height=6}
# visualizing residuals
calib_scale %>% 
  iso_plot_residuals(x = d13C, shape = id1, size = area44, trendlines = FALSE)
```
...
calib_scale_applied <- 
  ...
  iso_apply_calibration(true_d13C) %>% 
...
peak_table_calibrated %>% 
  filter(type == "sample") %>% 
  iso_plot_data(
    x = id1, y = true_d13C_pred, shape = str_extract(id1, "^\\w+"), 
